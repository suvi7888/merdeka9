...
<div class="table-responsive">
    ...
    <div class="col-md-6" style="padding-bottom: 50px">
        <div class="panel panel-warning">
            <div class="panel-heading">
                <h3 class="panel-title">Alasan Penolakan</h3> 
            </div>
            <div class="panel-body">
                <form class="form" action="#"> 
                    <div class="form-group">
                        <select class="form-control" id="bulanAlasan">
							<?php for($i=1; $i<=12; $i++){ ?>
                            <option value="<?php echo $i; ?>" <?php echo (int)date('m')-1 == $i ? 'selected':''; ?>><?php echo date('F', mktime(0, 0, 0, $i, 10)); ?></option>
							<?php } ?>
                        </select>
                ...
                <div id="pieAlasan" style="min-width: 310px; height: 400px; margin: 0 auto"></div>
            </div>
        </div>
    </div> 
...
$('#bulanAlasan').change(function(){
	$.ajax({
        method: 'POST',
        type: 'json',
        url: '<?php echo site_url('admin/chart/getDiagramPieTolak'); ?>',
        data: {
            tahun: tahun,
            bulan: $('#bulanAlasan').val(),
        },
        beforeSend: function( ) {
            // $('.card-loading').css('display','block');
        },
